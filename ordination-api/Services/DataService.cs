...
public class DataService
{
    ...
    public void SeedData() {
        ...
        if (patients[0] == null)
        {
            patients[0] = new Patient("121256-0512", "Tiny Tina", 24.4);
            patients[1] = new Patient("070985-1153", "Tunge Tommy", 121.2);
    ...
    public PN OpretPN(int patientId, int laegemiddelId, double antal, DateTime startDato, DateTime slutDato) {
        ...
        db.Patienter.Include(z => z.ordinationer).FirstOrDefault(x => x.PatientId == patientId).ordinationer.Add(pn);
    ...
    public DagligFast OpretDagligFast(int patientId, int laegemiddelId, 
        ...
        DagligFast dagligfast = new DagligFast(startDato, slutDato, db.Laegemiddler.FirstOrDefault(x => x.LaegemiddelId == laegemiddelId), antalMorgen, antalMiddag, antalAften, antalNat);

        db.Patienter.Include(z => z.ordinationer).FirstOrDefault(x => x.PatientId == patientId).ordinationer.Add(dagligfast);
    ...
    public DagligSkæv OpretDagligSkaev(int patientId, int laegemiddelId, Dosis[] doser, DateTime startDato, DateTime slutDato) {
        ...
        db.Patienter.Include(z => z.ordinationer).FirstOrDefault(x => x.PatientId == patientId).ordinationer.Add(dagligSkæv);
    ...
    {
        // Find the ordination by id
        var ordination = db.PNs.FirstOrDefault(x => x.OrdinationId == id);
        ...
        // Validate date
        if (dato.dato < DateTime.Now.Date)
        {
            return "Cannot apply ordination to a date in the past";
        }

        // Validate that the date is not already in the list
        if (ordination.dates.Contains(dato))
        {
            return "Ordination already applied to this date";
        }
        ...
        return "Ordination applied successfully";
    }
